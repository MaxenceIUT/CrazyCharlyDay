  ...
  const addToCart = async () => {
    if (props.categorie == 1) {
      ...
      if (!data || data.length == 0) {
        const { data, error } = await supabaseBrowser.from('commande').insert({ idUser: id, termine: false }).select();
        idCM = data[0].idCM;
      } else {
        idCM = data[0].idCM;
      }
      // setCart(prevState => [...cart, props]);
      const dataPanier = await supabaseBrowser.from('panier').select().eq('id_commande', idCM).eq('id_produit', props.id);
      
      if (dataPanier.data && dataPanier.data.length > 0) {
        const { data, error } = await supabaseBrowser.from('panier').update({ quantite: dataPanier.data[0].quantite + 1 }).eq('id_commande', idCM).eq('id_produit', props.id);
      } else {
        const panier = await supabaseBrowser.from('panier').insert({ id_commande: idCM, id_produit: props.id, quantite: 1 }).select();
        ...
        <LinkOverlay >
          <Card maxW='sm' className="hover:drop-shadow-lg">
            ...
            <CardFooter className="items-center justify-around">
              ...
              <Button variant='solid' colorScheme='red' onClick={props.trashedFonction}>
                <FaTrash />
              </Button>
            </CardFooter>
          </Card>
